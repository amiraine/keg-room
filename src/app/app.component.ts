...
//beer display
  taps: Taps[] = [
    ...
    new Taps('Level Let\'s Play Pilsner', 'Level Brewing', 6, 4.9,6),
    new Taps('Ninkasi Yours Truly Pale Ale','Ninkasi', 6, 4.3,124),
    new Taps('Logsdon Seizoen','Logsdon',5,7.5,98),
    new Taps('Pabst Blue Ribbon', 'Pabst Brewing',2,4.7,124),
    new Taps('Passion Statement','2 Towns Ciderhouse',5,5.2,40),
    new Taps('Wild Plum', 'WildCraft Cidery',5,7.5,81),
  ];
  //beer edits and adds
  refill(currentTap){
    if (currentTap.volume <=10){
      return "bg-danger";
    } else if (currentTap.volume <=50 && currentTap.volume >10){
      return "bg-warning";
    }
  }
  selectedTap = null;
